...
namespace mmapi {
    namespace tiny {
        ...
        void Player::processEvents(bool playMode) {
            ...
            for (; currentMsg &&
                   playTime >= currentMsg->time * 1000LL; currentMsg = currentMsg->next) {
                    ...
                    case TML_NOTE_OFF: //stop a note
                        if (!playMode) {
                            notes.remove_if([this](tml_message *m) {
                                return m->channel == currentMsg->channel &&
                                       m->key == currentMsg->key;
                            });
                        }
            ...
            if (!playMode) {
                for (const auto &m: notes) {
                    tsf_channel_note_on(synth, m->channel, m->key,
                                        static_cast<float>(m->velocity) / 127.0f);
                }
                notes.clear();
            }
        }
        ...
        Player::onAudioReady(oboe::AudioStream *audioStream, void *audioData,
            ...
            for (; numFrames > 0; numFrames -= sampleBlock, stream += sampleBlock *
